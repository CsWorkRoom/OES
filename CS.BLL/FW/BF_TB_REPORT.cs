...
namespace CS.BLL.FW
{
    ...
    public class BF_TB_REPORT : BBaseQuery
    {
        ...
        public class Entity
        {
            ...
            /// 报表名称
            /// </summary>
            [Field(IsNotNull = true, IsIndex = true, IsIndexUnique = true, Comment = "报表名称")]
        ...
        public static DataTable CheckSqlReturnDt(int dbID, string sqlCode, string inputJson)
        {
            ...
            try
            {
                dt = BF_DATABASE.Instance.ExecuteSelectSQL(dbID, sql, paramList, ref rows, 0, 1);
            }
            catch (Exception ex)
            {
                throw new Exception(string.Format(@"尝试执行SQL语句出错：{0},执行查询的sql语句为：{1}", ex.Message, sql));
            }
            return dt;
        }
