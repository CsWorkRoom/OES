...
namespace CS.BLL.FW
{
    ...
    public class BF_DEPARTMENT : BBaseQuery
    {
        ...
        #region 根据根节点获得至指定层级的所有子节点
        /// <summary>
        /// 递归方法：根据根节点获得至指定层级的所有子节点
        /// </summary>
        /// <param name="deptCode">起点code编号</param>
        /// <param name="level">截至层级:0=省，1=市，2=区县</param>
        /// <returns>返回满足的所有子节点</returns>
        public List<Entity> GetDepartmentList(Entity rootDept, int level)
        {
            List<Entity> deptList = new List<Entity>();
            if (rootDept != null && rootDept.DEPT_LEVEL < level)
            {
                var childList = Instance.GetList<Entity>("P_CODE=?", rootDept.DEPT_CODE);
                if (childList != null && childList.Count > 0)
                {
                    deptList.AddRange(childList);
                    foreach (var child in childList)
                    {
                        var ccList = GetDepartmentList(child, level);
                        if (ccList != null && ccList.Count > 0)
                        {
                            deptList.AddRange(ccList);
                        }
                    }
                }
            }
