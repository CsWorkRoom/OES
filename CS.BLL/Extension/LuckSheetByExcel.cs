    ...
    public class LuckSheetByExcel
    {
        ...
        public void exportLuckySheetXlsxByNPOI()
        {
            ...
            for (int sheetIndex = 0; sheetIndex < _data.Length(); sheetIndex++)
            {
                ...
                if (jsonObject.Opt("visibledatarow") == null) continue;
                ...
                if (rowObjectList != null && rowObjectList.Length() > 0)
                {
                    for (int i = 0; i < rowObjectList.Length(); i++)
                    {
                        ...
                        if (colObjectList != null && colObjectList.Length() > 0)
                        {
                            ...
                            {
                                if (columnlenObject != null && columnlenObject.Opt(j.ToString()) != null)
                                {
                ...
                //设置值,样式
                setCellValue(dataObjectList, celldataObjectList, borderInfoObjectList, sheet, excel);
        ...
        public void setCellValue(JSONArray dataObjectList, JSONArray jsonObjectList, JSONArray borderInfoObjectList, XSSFSheet sheet, XSSFWorkbook workbook)
        {
            ...
            for (int index = 0; index < jsonObjectList.Length(); index++)
            {
                ...
                var _or = _object.GetInt("r");
                var _oc = _object.GetInt("c");

                var _dor = dataObjectList.GetJSONArray(_or);
                var _doc = _dor.Get(_oc);
                if (_doc == null || _doc == "")
                {
                    continue;
                }
                ...
                if (sheet.GetRow((int)_object.Get("r")) != null && sheet.GetRow((int)_object.Get("r")).GetCell((int)_object.Get("c")) != null)
                {
                    ...
                    {//如果有公式，设置公式
                        ...
                        cell.SetCellFormula(value.Substring(1, value.Length - 1));//不需要=符号,例：INT(12.3)
                    }
                    //合并单元格与填充单元格颜色
                    ...
                    //设置垂直水平对齐方式
                    int vt = jsonObjectValue.Opt("vt") == null ? 1 : (int)jsonObjectValue.Get("vt");//垂直对齐	 0 中间、1 上、2下
                    int ht = jsonObjectValue.Opt("ht") == null ? 1 : (int)jsonObjectValue.Get("ht");//0 居中、1 左、2右
                    switch (vt)
                    {
                        case 0:
        ...
        public void setBorder(JSONArray borderInfoObjectList, XSSFWorkbook workbook, XSSFSheet sheet)
        {
            ...
            //            if (borderValueObject.Opt("l") != null)
            //            {
            ...
            //                }
            //            }
            //            if (borderValueObject.Opt("r") != null)
            //            {
            ...
            //                }
            //            }
            //            if (borderValueObject.Opt("t") != null)
            //            {
            ...
            //                }
            //            }
            //            if (borderValueObject.Opt("b") != null)
            //            {
            //                if (borderValueObject.Get("b") != null)
            //                {
