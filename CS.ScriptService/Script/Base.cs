...
namespace CS.ScriptService.Script
{
    ...
    public class Base
    {
                    ...
                    using (IDataReader reader = dbHelper.ExecuteReader(sql))
                    {
                        ...
                        while (Main.IsRun && reader.IsClosed == false && isCanRead)
                        {
                            ...
                            if (i >= pageSize || isCanRead == false)
                            {
                                ...
                                //避免过多数据压到内存
                                //(为避免oracle查询数据量过大，期间进行了commit造成的快照过旧问题,可根据配置适当调大限制数量)
                                while (_queueAsyncLoadTask.Count > 5)
                                {
                                    if (_isError == true)
                                    {
            ...
            while (Main.IsRun)
            {
                ...
                try
                {
                    ...
                    using (BDBHelper dbHelperDest = new BDBHelper(dbTypeDest.ToString(), asyncTask.DestDBServer.IP, asyncTask.DestDBServer.PORT, asyncTask.DestDBServer.USER_NAME, asyncTask.DestDBServer.PASSWORD, asyncTask.DestDBServer.DB_NAME, asyncTask.DestDBServer.DB_NAME))
                    {
                        ...
                        if (rc < Main.UseFileModeRows)
                        {
                            BLog.Write(BLog.LogLevel.DEBUG, string.Format(@"即将第【{0}】页查询结果将以内存方式导入到库", asyncTask.PageIndex));
                            n = dbHelperDest.LoadDataInDataTable(asyncTask.DestTableName, asyncTask.Table);
                        }
                        else
                        {
                            BLog.Write(BLog.LogLevel.DEBUG, string.Format(@"即将第【{0}】页查询结果将以文件方式导入到库", asyncTask.PageIndex));
        ...
        public int load_txt_to_db(string destTableName, string destDBName, string txtPath,string fieldsTerminated=null, bool isDeleteFile = true)
        {
            #region 基础校验
            if (_isError == true)
            {
            ...
            try
            {
                ...
                {
                    return dbHelper.LoadTxtToTable(destTableName, txtPath, fieldsTerminated,isDeleteFile);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// 获得指定文件的属性
        /// 当前收录属性：name(文件名),size(文件大小),time(创建时间)
        /// </summary>
        /// <param name="fileName">文件完整物理路径</param>
        /// <returns></returns>
        public Dictionary<string, string> get_file_info(string fileName)
        {
            Dictionary<string, string> dic = new Dictionary<string, string>();
            try
            {
                log("开始获得文件属性，源文件路径为：" + fileName);
                FileInfo info = new FileInfo(fileName);
                dic.Add("name", info.Name);
                dic.Add("size", info.Length.ToString());
                dic.Add("time", info.CreationTime.ToString("yyyyMMddHHmmss"));
                log("文件属性获取成功！");
            }
            catch (Exception ex)
            {
                WriteErrorMessage(string.Format(@"获取文件[{0}]属性失败:{1}", fileName, ex.Message));
                return null;//返回空
            }
            return dic;
        }
        ...
        /// <returns>返回生成的文件完整物理路径</returns>
        public string export_text(string fileName, string sql, string separate = "\t")
        {
            string endFile = "";//初始化最终文件完整物理路径
            ...
            {
                //txt文件临时目录
                string tempPath = CS.Base.Config.BConfig.BaseDirectory + "txtTemp";
                ...
                using (BDBHelper dbHelper = new BDBHelper(dbType.ToString(), _dbServer.IP, _dbServer.PORT, _dbServer.USER_NAME, _dbServer.PASSWORD, _dbServer.DB_NAME, _dbServer.DB_NAME))
                {
                    ...
                    log(string.Format("在【{0}】执行SQL语句成功,记录数为：{1}", _dbServer.NAME, dt.Rows.Count), sql);

                    //将查询结果导出到临时文件
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        endFile = tempPath + @"\" + fileName;
                        log("开始生成txt文件，文件路径为：" + endFile);
                        dbHelper.WriteDataTableIntoFile(dt, endFile, separate);
                        log("txt文件生成成功！");
        ...
        /// <returns></returns>
        public bool ftp_upload(string ftpServerIp, string user, string pwd, string directory, string fileName, out string errMsg)
        {
            errMsg = "";
            try
            {
                log("开始上传文件到ftp，源文件路径：" + fileName);

                FtpWeb ftp = new FtpWeb(ftpServerIp, directory, user, pwd);
                if (ftp.Upload(fileName, out errMsg))
                {
                    log("文件上传到ftp服务器成功！");
                    return true;
                }
                ...
                {
                    errMsg = "文件上传到ftp服务器失败：" + errMsg;
                    WriteErrorMessage(errMsg, 3, "");
                    return false;
                }
            }
            catch (Exception ex)
            {
                errMsg = "文件上传到ftp失败：" + ex.Message;
                WriteErrorMessage(errMsg, 3, "");
                return false;
            }
        }
        ...
        /// <param name="fileName">文件完整物理路径</param>
        /// <returns></returns>
        public bool delete_file(string fileName)
        {
            bool resBool = false;
            try
            {
                ...
                else WriteErrorMessage("未找到文件：" + fileName);
            }
            catch (Exception ex)
            {
                WriteErrorMessage("删除文件失败：" + ex.Message);
            }
            return resBool;
        }
        #endregion
        ...
        /// 根据地市用户名获得ID_NO
        /// </summary>
        /// <param name="cityDbUserName"></param>
        /// <returns></returns>
        public int get_city_id_no(string cityDbUserName)
        {
            switch (cityDbUserName)
            {
                case "CD_USER":
