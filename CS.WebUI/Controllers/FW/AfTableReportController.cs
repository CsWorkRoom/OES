...
namespace CS.WebUI.Controllers.FW
{
    ...
    public class AfTableReportController : ABaseController
    {
            ...
            foreach (DataColumn col in dt.Columns)
            {
                ...
                else
                {
                    ...
                    if (dicSystemFields.ContainsKey(en))
                    {
                        ...
                        model.IS_SORT = dicSystemFields[en].IS_SORT;
                        model.SHOW_WIDTH = dicSystemFields[en].SHOW_WIDTH;
                        model.SORT = dicSystemFields[en].SORT;
                    }
                    else
                    {
                        model.CN_NAME = en;
        ...
        public ActionResult Show(int id)
        {
            ...
            {
                BLog.Write(BLog.LogLevel.WARN, "查询报表[" + id + "]出现错误：" + ex.ToString());
                return ShowAlert("查询报表出错，详见运行日志。");
            }
        }
        ...
        {
            string code = SystemSession.TransParams(entity.TOP_CODE);
        ...
        private string GenerateTableRender(BF_TB_REPORT.Entity entity, List<BF_TB_REPORT_EVENT.Entity> rowEventList, string queryString, int footHeight = 0)
        {
            ...
            else
            {
                ...
                if (isSort) //是否排序
                    fieldList.Sort((x, y) => -y.SORT.CompareTo(x.SORT));//排序
        ...
        {
            string code = SystemSession.TransParams(entity.BOTTOM_CODE);
            code = Functions.Instance.ExecuteFunction(code);
