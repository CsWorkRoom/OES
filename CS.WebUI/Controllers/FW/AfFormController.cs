    ...
    public class AfFormController : ABaseController
    {
        ...
        private BF_FORM.Entity GetFormFields(BF_FORM.Entity entity)
        {
            ...
            foreach (BF_FIELD.Entity item in list)
            {
                ...
                {
                    if (item.EN_NAME == info.EN_NAME && item.FIELD_DATA_TYPE == info.FIELD_DATA_TYPE)
                    {
                        isSelect = true;
                        break;
                    }
        ...
        public JsonResult Edit(BF_FORM.Entity entity)
        {
            ...
            catch (Exception ex)
            {
                ...
                result.Message = "提交出错，详见日志。";
                BLog.Write(BLog.LogLevel.WARN, "表单【" + entity.ID + "】提交出错，" + ex.ToString());
            }
