...
namespace CS.WebUI.Controllers.FW
{
    ...
    public class AfBulletinController : ABaseController
    {
        ...
        public ActionResult Edit(int id = 0)
        {
            ...
            //string [] filePath = AttachmentPath.Split('\\');
            //string fictitiousFile = filePath[filePath.Length-1];//虚拟文件目录
            //string url = Request.Url.Host + "/" + MvcApplication.ApplicationPath+"/"+ fictitiousFile;
            foreach (var item in fileList)
            {
                fileIds += (fileIds == "" ? "" : "|") + item.FILE_PATH;
                ...
                fileModel.FILE_NAME = item.FILE_PATH;
                list.Add(fileModel);
            }
        ...
        public ActionResult Edit(BLL.FW.BF_BULLETIN.Entity model, FormCollection collection, HttpPostedFileBase file)
        {
            ...
            if (file != null)
            {
                ...
                try
                {
                    ...
                    path = new DirectoryInfo(AttachmentPath).FullName;
                    string saveName = string.Format("{0}-{1}_{2}", DateTime.Now.ToString("yyyyMMdd-HHmmss"), SystemSession.UserName, fileName);
                    path = path + "\\" + saveName;
                    file.SaveAs(path);
        ...
        public ActionResult deleteFile(int id)
        {
            ...
            try
            {
                ...
                string fullName = AttachmentPath + "/" + model.FILE_PATH;
        ...
        {          
            ...
            else
            {
                ...
                int index = model.FILE_PATH.IndexOf('_');
                string name = model.FILE_PATH.Substring(index + 1);
        ...
        public ActionResult Download(int id)
        {
            ...
            try
            {
                ...
                //下载单个文件
                if (string.IsNullOrWhiteSpace(model.FILE_PATH) == false)
                {
                    string fullName = AttachmentPath + "/" + model.FILE_PATH;
                    if (System.IO.File.Exists(fullName) == false)
                    {
