...
$(function () {
    layui.use(['form', 'element', 'layer', 'jquery'], function () {
        ...
        form.on('select(TOPYYYYMM)', function (data) {
            GetTopSumChart();
        });
        GetTopSumChart();//顶部饼状图
        //历史薪酬报表情况
        GetHistoryChart();
...
function GetRegionChart() {
    var date = $("#YYYYMM").val();
    var titleStr = "地区薪酬分析";
    if ($("#DepartmentLevel").val() == 2 || $("#DepartmentLevel").val() == "***") {
        titleStr = "客户经理薪酬分析";
    }
...
function GetHistoryChart() {
    var DepartmentLevel = $("#DepartmentLevel").val();
    $.post("../Home/GetHistory?DepartmentLevel=" + DepartmentLevel, function (data) {
        var html = "";
        if (data.length > 0) {
            var dataInfo = data.split('◎');
            if (dataInfo.length <= 1) {
...
//远程获取顶部报表图
function GetTopSumChart() {
    var date = $("#TOPYYYYMM").val();
    var DepartmentLevel = $("#DepartmentLevel").val();
    $.post("../Home/GetTopSumChart?date=" + date + "&DepartmentLevel=" + DepartmentLevel, function (data) {
        var html = "";
        if (data.length > 0) {
            var objData = eval("(" + data + ")");
            var qpfx = objData.qpFee;
            var jjfx = objData.jjFee;
            var fxfx = objData.fxFee;
            var sumFee = objData.sumFee;
            LoadTopChart(qpfx, jjfx, fxfx, sumFee);
        }
    });
}

///加载顶部的报表
function LoadTopChart(qpfx, jjfx, fxfx, sumFee) {
    $("#sumTopFee").html(sumFee);
