...
function chartHelp() {
    layui.use(['layer'], function () {
        ...
        content += "GetFieldValueArray(fieldName)：根据字段名获得集合(不剔重)方法。从dataArr集合中获得指定字段的数据集合。如：GetFieldValueArray(\"USER_NAME\"),返回：['用户一','用户二']<br/>";
...
function analysisSql() {
    var data = $.ajax({
        type: "post",
        data: { DB_ID: $("#DB_ID").val(), SQL_CODE: $("#SQL_CODE").val(), QUERY_STRING:"[]" },
