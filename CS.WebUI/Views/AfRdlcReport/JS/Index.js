...
layui.use(["table", "jquery"], function () {
    ...
    table.on('tool(tablefilter)', function (obj) {
        ...
        if (layEvent === 'edit') {
            OpenTopWindow("编辑图形报表", 0, 0, "../AfRdlcReport/Edit?id=" + data.ID);
        } else if (layEvent === 'del') {
            layer.confirm('真的删除行么', function (index) {
                layer.close(index);
                var url = "../AfRdlcReport/Delete?id=" + data.ID;
...
//添加
function Add() {
    OpenWindow("添加图形报表", 0, 0, "../AfRdlcReport/Edit?id=0");
}
...
//导出
function ExportFile() {
    url = "../AfRdlcReport/ExportFile?name=" + $.trim($("#name").val());
    window.location = url;
}